...
- **Porta do administrador**: um painel reservado (aberto por uma combinação de teclas no computador ou por um **toque secreto** no celular) onde se cadastra/remove nomes, se **gera** o arquivo da lista para commit e se **importa** um arquivo existente (por arquivo ou colando o JSON) para continuar a administração em outra máquina. Os nomes reais viajam **cifrados** dentro do próprio arquivo (só a senha de administrador abre), então o admin **recupera os nomes** em qualquer máquina ao entrar no painel — o público continua vendo apenas hashes.
